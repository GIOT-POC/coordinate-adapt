    ...
    combinations.forEach(function(array) {
        ...
        dbClient.search({
            ...
            if (!err) {
                ...
                if (hits.length != 0) {
                    for (var hIdx in hits) {
                        ...
                        var posArray = src.position;

                        for (var pIdx in posArray) {
                            var pos = posArray[pIdx];
                            var key = pos.GPS_N + '-' + pos.GPS_E;
                            var gwNum = Math.floor(src.characteristic.length / 16);
                            var point = gwNum / (combNum[dataArray.length - gwNum]);
                            ...
                            var posInfo = {pos: pos, point: tmpResults[key]};
            ...
            if (taskNum == 0) {
                ...
                if (results.length > 1) {
                    resIdx = Math.floor(Math.random() * results.length);
                }

                callback(null, {GPS_N: results[resIdx].pos.GPS_N, GPS_E: results[resIdx].pos.GPS_E});
            }
        });
    });
}

function find_bak(tbID, tolerance, dataArray, callback) {
    var type = tbID? tbID: 'gps-history';
...
function remove(tbID, position, callback) {
    var filters = [ { term: { 'position.GPS_N': position.GPS_N, 'position.GPS_E': position.GPS_E }}];
